...
contract NiveAgentVault is ERC4626 {
    // ────────────────────────────────
    //  State
    // ────────────────────────────────
    ...
    function submitProposal(bytes calldata data) external returns (bytes32 proposalId) {
        require(msg.sender == agentId, "NiveAgentVault: not agent");
    ...
    function executeProposal(bytes32 proposalId, bytes calldata result) external {
        require(msg.sender == agentId, "NiveAgentVault: not agent");
        require(!executedProposals[proposalId], "NiveAgentVault: already executed");
    ...
    function setPerformanceFee(uint256 _fee) external {
        require(msg.sender == agentId, "NiveAgentVault: not agent");
        require(_fee <= 5000, "NiveAgentVault: fee too high"); // max 50%
        performanceFee = _fee;
    }
    ...
    function deposit(uint256 assets, address receiver) public override returns (uint256) {
        require(acceptingDeposits, "NiveAgentVault: not accepting deposits");
