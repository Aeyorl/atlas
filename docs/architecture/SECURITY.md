# Nive Protocol Security Model

## Multi-Layer Security

1. **Guardian Network** — guardians attest cross-ecosystem bridge messages;
   delivery requires a 2-of-5 committee quorum (`NiveBridge.GUARDIAN_QUORUM`)
...
| Guardian stake custody | 10,000 NIVE locked | ⚠️ stake is recorded, **not token-locked**; slashing is accounting-only |
...
- Delivery on the target chain is accepted by `NiveBridge.deliverMessage` only
